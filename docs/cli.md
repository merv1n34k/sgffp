...
# 13:  1 [*]
#
# * undecoded but known blocks (auto-generated by SnapGene)

sff check plasmid.dna -l -d
# (also dumps hex of any [NEW] blocks)
```

Blocks are categorized:
- No marker — parsed and modeled
- `[*]` — known but intentionally not parsed (SnapGene auto-regenerates blocks 2, 3, 13)
- `[NEW]` — genuinely unknown block type (please report these)
