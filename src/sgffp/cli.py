...
def cmd_check(args):
    ...
    try:
        ...
        while True:
            ...
            if (
                block_type not in PARSED_BLOCKS
                and block_type not in UNDECODED_BLOCKS
    ...
    if args.list:
        has_undecoded = any(bt in UNDECODED_BLOCKS for bt in found_blocks)
        has_new = bool(unknown)

        for block_type in sorted(found_blocks.keys()):
            count = len(found_blocks[block_type])
            if block_type in UNDECODED_BLOCKS:
                marker = "[*]"
            elif block_type not in PARSED_BLOCKS:
                marker = "[NEW]"
            else:
                marker = ""
            print(f"{block_type:>2}: {count:>2} {marker}")

        if has_undecoded or has_new:
            print()
        if has_undecoded:
            print("* undecoded but known blocks (auto-generated by SnapGene)")
        if has_new:
            print(f"NEW unknown blocks: {sorted(unknown)}")
