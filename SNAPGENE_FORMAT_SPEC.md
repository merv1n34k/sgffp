...
| `upstreamStickiness` | string | Upstream sticky end length (e.g., `"-1"`) |
| `downstreamStickiness` | string | Downstream sticky end length (e.g., `"-1"`) |
...
**Attribute order convention:** SnapGene expects attributes in this order:
...
- `<InputSummary>` — **Required on every non-leaf node** (SnapGene segfaults without it). Contains: `manipulation`, `val1`, `val2`, `name1`/`siteCount1` (enzymes), or strain/methylation info. An empty `<InputSummary/>` is valid.
- `<Oligo>` — Primer used in PCR: `name`, `sequence`, `phosphorylated` (optional, `"0"` or `"1"`)
- `<Parameter>` — Key-value pairs: `name`, `val`
- `<RegeneratedSite>` — Restriction site regenerated by cloning: `name`, `pos`, `siteCount`
- `<HistoryColors>` — Strand coloring for history map display
- `<Features>` — Snapshot of sequence features at this history state
